...
from asu.onnxocr.onnx_paddleocr import ONNXPaddleOcr
...
class My_TS:
    ...
    def ocr_one_row(self, img, box=None):
        if box is None:
            ...
            return self.ts.text_recognizer([img[box[2] : box[3], box[0] : box[1]]])[0][
                0
            ]
    ...
    def merge(self, text):
        ...
        for i in range(1, len(text)):
            if (
                abs(text[i]["box"][2] - merged["box"][2]) <= 10
                and abs(text[i]["box"][3] - merged["box"][3]) <= 10
                and abs(text[i]["box"][0] - merged["box"][1]) <= 35
            ):
    ...
    def find_with_box(self, box=None, redundancy=10, forward=0, mode=0):
        if forward and box is not None:
            ...
            if box[3] == 540 or box[3] == 350 and self.father.debug:
                ...
                cv.imwrite(
                    "img/" + tm + ".jpg",
                    self.father.screen[box[2] : box[3], box[0] : box[1]],
                )
                cv.imwrite(
                    "img/" + tm + "w.jpg",
                    self.filter_non_white(
                        self.father.screen[box[2] : box[3], box[0] : box[1]], mode=mode
                    ),
                )
        ans = []
        for res in self.res:
